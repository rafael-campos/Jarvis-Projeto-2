...
def listen_print_loop(responses):
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            fileObject = open("podeFalarAg.txt", "r")
            data = fileObject.read()
            

            bugJa = "-" + transcript.replace(" ", "") + "-"
            if "sim" in data:
                #                                 #
                ...
                if(x == "nao e calculo"):
                    ...
                    if "Obrigado" in transcript or "obrigado" in transcript or "obrigada" in transcript:
                        podeFalar = "Obrigado"
                        CREATE_NO_WINDOW = 0x08000000
                        ctypes.windll.kernel32.SetConsoleTitleA("My New Title")
                        p = subprocess.Popen(["python", "speak.py", "Por nada, se precisar é só chamar pelo meu nome."], creationflags=subprocess.CREATE_NO_WINDOW)
                        ...
                        CREATE_NO_WINDOW = 0x08000000
                        ctypes.windll.kernel32.SetConsoleTitleA("My New Title")
                        from automacao import apagarLuzQuarto

                        p = subprocess.Popen(["python", "speak.py", apagarLuzQuarto()], creationflags=subprocess.CREATE_NO_WINDOW)
                    
                    elif "acende" in transcript and "luz" in transcript and "quarto" in transcript or "liga" in transcript and "luz" in transcript and "quarto" in transcript     or "Acende" in transcript and "luz" in transcript and "quarto" in transcript or "Liga" in transcript and "luz" in transcript and "quarto" in transcript or "atende" in transcript and "luz" in transcript and "quarto" in transcript:
                        from automacao import ligarLuzQuarto
                        xd = ligarLuzQuarto()
                        CREATE_NO_WINDOW = 0x08000000
                        ctypes.windll.kernel32.SetConsoleTitleA("My New Title")
                        p = subprocess.Popen(["python", "speak.py", xd], creationflags=subprocess.CREATE_NO_WINDOW)
